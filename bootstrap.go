...
func Bootstrap(pi europi.Hardware, options ...BootstrapOption) error {
	...
	bootstrapInitializeComponents(ctx, &config, e)
...
func bootstrapInitializeComponents(ctx context.Context, config *bootstrapConfig, e europi.Hardware) {
	...
	if config.enableNonPicoWebSocket {
		config.nonPicoWSApi = europi.ActivateNonPicoWS(ctx, e)
	}
...
func bootstrapDestroy(config *bootstrapConfig, e europi.Hardware, reason any) {
	...
	if config.enableNonPicoWebSocket && config.nonPicoWSApi != nil {
		_ = config.nonPicoWSApi.Shutdown()
	}
